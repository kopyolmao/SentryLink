...
$selectedEventInfo = $selectedEvent > 0
    ? db_fetch_one($conn, 'SELECT id, title, event_date FROM events WHERE id = ? AND deleted_at IS NULL', 'i', [$selectedEvent])
    : null;

$scanCount = $selectedEvent > 0
    ? (int) db_scalar($conn, 'SELECT COUNT(*) FROM admissions WHERE event_id = ?', 'i', [$selectedEvent])
    : 0;

shell_start('SentryLink | Scanner', $user, 'ssg', 'scanner', 'QR Scanner', 'Select an ongoing event, then validate student QR codes.');
?>
<style>
.scanner-shell {
    display: flex;
        ...
        <div class="col-md-8">
            ...
            <select class="form-select" name="event_id" onchange="this.form.submit()">
                ...
                <?php foreach ($events as $event): ?>
                    <option value="<?= $event['id'] ?>" <?= $selectedEvent === (int) $event['id'] ? 'selected' : '' ?>>
        ...
        <div class="col-md-4">
            <?php if ($selectedEventInfo): ?>
                <div class="metric py-2">
                    <div class="text-secondary">Admissions for Event</div>
                    <div class="value" style="font-size: 26px;"><?= $scanCount ?></div>
                </div>
            <?php endif; ?>
        </div>
    </form>
    ...
    <div class="scanner-layout">
        <div class="scanner-preview-card">
            ...
            <div class="scanner-controls">
                <button class="btn btn-primary" type="button" onclick="startScanner()">Start Camera</button>
                <button class="btn btn-outline-light" type="button" onclick="stopScanner()">Stop Camera</button>
            </div>
        </div>
...
function statusClass(status) {
    if (status === "admitted") return "scan-status scan-status-admitted";
    if (status === "duplicate") return "scan-status scan-status-duplicate";
