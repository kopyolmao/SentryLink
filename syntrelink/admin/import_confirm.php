...
$event = db_fetch_one($conn, 'SELECT * FROM events WHERE id = ?', 'i', [$eventId]);
        ...
        foreach ($validRows as $row) {
            db_execute(
                ...
                "INSERT INTO tickets (user_id, event_id, ticket_code, receipt_id, payment_status, issued_at, updated_at)
                 VALUES (?, ?, ?, ?, 'paid', NOW(), NOW())",
                'iiss',
                [(int) $row['user_id'], $eventId, generate_ticket_code($conn), $row['receipt_id']]
            );
            $inserted++;
            notify_user($conn, (int) $row['user_id'], 'Ticket Ready', 'Your ticket for ' . $event['title'] . ' is now active.', 'success');
        }

        audit_log($conn, (int) $user['id'], 'CSV_RECEIPT_IMPORT', 'event', $eventId);
        $_SESSION['import_success'] = ['inserted' => $inserted, 'event_title' => $event['title']];
        unset($_SESSION['import_data']);
        redirect_to('admin/tickets/import-receipts/success');
    }
}

shell_start('SentryLink | Confirm Import', $user, 'admin', 'import', 'Confirm Import', 'Review the validated rows before creating tickets.');
?>
...
<div class="panel">
        ...
        <div class="table-wrap">
            <table class="table table-dark align-middle">
                ...
                <?php foreach ($validRows as $row): ?>
                    <tr><td><?= h($row['receipt_id']) ?></td><td><?= h($row['student_no']) ?></td><td><span class="badge text-bg-success">Valid</span></td></tr>
                <?php endforeach; ?>
                </tbody>
            </table>
        </div>
    ...
    <?php if ($invalidRows): ?>
        ...
        <ul class="list-soft">
            <?php foreach ($invalidRows as $row): ?><li><?= h($row) ?></li><?php endforeach; ?>
        </ul>
    <?php endif; ?>
