...
<div class="panel">
    <form method="POST" class="row g-3">
        <div class="col-md-3"><label class="form-label">Role</label><select class="form-select" name="role"><option value="">All roles</option><?php foreach (['student', 'ssg', 'admin', 'director', 'cashier'] as $role): ?><option value="<?= h($role) ?>"><?= h(shell_role_label($role)) ?></option><?php endforeach; ?></select></div>
